...
package com.sirga.studio.ui.settings
...
import com.sirga.studio.engine.settings.AudioSettings
...
import com.sirga.studio.engine.settings.VideoSettings
...
import com.sirga.studio.ui.studio.Chips
import com.sirga.studio.ui.studio.LabeledSlider
import com.sirga.studio.ui.studio.SwitchRow
import com.sirga.studio.ui.studio.ToolButton
import com.sirga.studio.ui.theme.Sirga

private const val REPO_URL = "https://github.com/lopezdevf/sirga-studio"
    ...
    Dialog(onDismissRequest = onClose, properties = DialogProperties(usePlatformDefaultWidth = false, decorFitsSystemWindows = false)) {
        Column(Modifier.fillMaxSize().background(Sirga.colors.ink).safeDrawingPadding().imePadding()) {
            Row(Modifier.fillMaxWidth().height(56.dp).padding(horizontal = 4.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Ajustes", color = Sirga.colors.textHigh, modifier = Modifier.padding(start = 8.dp).weight(1f))
            }
            Column(
                Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp),
                verticalArrangement = Arrangement.spacedBy(12.dp),
            ) {
                if (busy) Callout("En directo o grabando: resolución, fps, códec y frecuencia de audio quedan bloqueados.", Sirga.colors.record)
...
private fun VideoSection(s: StudioSettings, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    ...
    Text(recommendation(v), style = Sirga.numeric, color = Sirga.colors.textLow)
    ...
    if (v.codec == VideoCodecChoice.H265) {
        Callout("H.265 solo se usa si todos los destinos activos lo admiten (YouTube o servidores propios); si no, se emite en H.264.", Sirga.colors.textMid)
    }
    ...
    Text("Una vista previa más lenta ahorra batería y calor; no afecta al directo.", style = Sirga.numeric, color = Sirga.colors.textLow)
    SwitchRow(
...
private fun ThermalSection(s: StudioSettings, thermal: ThermalProfile, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    val t = s.thermal
    Header("TEMPERATURA")
    Callout("Estado actual: ${thermal.level.label}" + (thermal.message?.let { "\n$it" } ?: ""), if (thermal.throttled) Sirga.colors.record else Sirga.colors.textMid)
...
private fun RecordingSection(s: StudioSettings, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    ...
    Text("La grabación usa el mismo codificador que el directo: calidad idéntica y sin calor extra.", style = Sirga.numeric, color = Sirga.colors.textLow)
}
...
private fun AboutSection(onOpen: (String) -> Unit) {
    ...
    Header("ACERCA DE")
    Text("Sirga Studio $version", color = Sirga.colors.textHigh)
    Text(
    ...
    Text(
        "Sirga Studio™ y su logotipo son marcas de Miguel Lopez y no están incluidos en la licencia: " +
            "las versiones modificadas deben usar otro nombre y otro logotipo.",
    ...
    LinkRow("Política de marca", "$REPO_URL/blob/main/TRADEMARKS.md", onOpen)
    LinkRow("Reportar un error", "$REPO_URL/issues", onOpen)
    Row(
        Modifier
            .fillMaxWidth()
            .background(Sirga.colors.panel, RoundedCornerShape(Sirga.metrics.radius))
            .clickable { onOpen(SPONSOR_URL) }
            .padding(16.dp),
        verticalAlignment = Alignment.CenterVertically,
    ) {
        Icon(Icons.Outlined.Favorite, null, tint = Sirga.colors.live, modifier = Modifier.size(24.dp))
        Column(Modifier.padding(start = 12.dp).weight(1f)) {
            Text("Apoyar el proyecto", color = Sirga.colors.textHigh)
            Text("Sirga Studio es gratis y sin anuncios. Tu apoyo en GitHub Sponsors mantiene el desarrollo.", style = Sirga.numeric, color = Sirga.colors.textLow)
        }
    }
}
...
private fun LinkRow(title: String, url: String, onOpen: (String) -> Unit) {
    Row(Modifier.fillMaxWidth().clickable { onOpen(url) }.padding(vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
        Text(title, color = Sirga.colors.accentText, modifier = Modifier.weight(1f))
...
private fun Header(text: String) {
    Text(text, style = Sirga.panelLabel, color = Sirga.colors.accentText, modifier = Modifier.padding(top = 12.dp))
}
