...
fun MixerPanel(
    ...
    if (state.audio.isEmpty()) {
        Text("No hay fuentes de audio. Añade un micrófono o el audio interno desde Fuentes.", color = Senda.colors.textLow, modifier = modifier.padding(16.dp))
        return
    }
    LazyColumn(modifier, contentPadding = PaddingValues(12.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        monitorStatus?.let { status -> item { Callout(status, Senda.colors.record) } }
            ...
            Column {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Column(Modifier.padding(start = 8.dp).weight(1f)) {
                        Text(source.name, color = Senda.colors.textHigh, maxLines = 1)
                        deviceLabel(source, inputs)?.let { Text(it, style = Senda.numeric, color = Senda.colors.textLow, maxLines = 1) }
                    }
                    if (channel.monitoring != MonitoringMode.Off) {
                        Icon(Icons.Outlined.Headphones, "Monitorizado", tint = Senda.colors.accentText, modifier = Modifier.size(16.dp))
                    }
                    Text(
                        if (channel.muted) "MUTE" else String.format(Locale.ROOT, "%+.1f dB", channel.gainDb),
                        style = Senda.numeric,
                        color = if (channel.muted) Senda.colors.live else Senda.colors.textMid,
                    ...
                    ToolButton(
                        ...
                        tint = if (channel.muted) Senda.colors.live else Senda.colors.textMid,
                    )
                    ToolButton(Icons.Outlined.Tune, "Ajustes de ${source.name}", { onProperties(channel.sourceId) })
                }
                errors[channel.sourceId]?.let { Text(it, style = Senda.numeric, color = Senda.colors.record, modifier = Modifier.padding(top = 4.dp)) }
                ...
                Slider(
                    ...
                    colors = SliderDefaults.colors(
                        thumbColor = Senda.colors.textHigh,
                        activeTrackColor = if (channel.muted) Senda.colors.textLow else Senda.colors.accent,
                        inactiveTrackColor = Senda.colors.line,
                    ),
