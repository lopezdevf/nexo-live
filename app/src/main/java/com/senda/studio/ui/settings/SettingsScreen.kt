...
package com.senda.studio.ui.settings
...
import com.senda.studio.engine.settings.AudioSettings
import com.senda.studio.engine.settings.StudioSettings
import com.senda.studio.engine.settings.ThermalPolicy
import com.senda.studio.engine.settings.TransitionType
import com.senda.studio.engine.settings.VideoSettings
import com.senda.studio.engine.thermal.ThermalLevel
import com.senda.studio.engine.thermal.ThermalProfile
...
import com.senda.studio.ui.studio.Chips
import com.senda.studio.ui.studio.LabeledSlider
import com.senda.studio.ui.studio.SwitchRow
import com.senda.studio.ui.studio.ToolButton
import com.senda.studio.ui.theme.Senda

private const val REPO_URL = "https://github.com/lopezdevf/senda-studio"
    ...
    Dialog(onDismissRequest = onClose, properties = DialogProperties(usePlatformDefaultWidth = false, decorFitsSystemWindows = false)) {
        Column(Modifier.fillMaxSize().background(Senda.colors.ink).safeDrawingPadding().imePadding()) {
            Row(Modifier.fillMaxWidth().height(56.dp).padding(horizontal = 4.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Ajustes", color = Senda.colors.textHigh, modifier = Modifier.padding(start = 8.dp).weight(1f))
            }
            Column(
                Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp),
                verticalArrangement = Arrangement.spacedBy(12.dp),
            ) {
                if (busy) Callout("En directo o grabando: resolución, fps, códec y frecuencia de audio quedan bloqueados.", Senda.colors.record)
...
private fun VideoSection(s: StudioSettings, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    ...
    Text(recommendation(v), style = Senda.numeric, color = Senda.colors.textLow)
    ...
    if (v.codec == VideoCodecChoice.H265) {
        Callout("H.265 solo se usa si todos los destinos activos lo admiten (YouTube o servidores propios); si no, se emite en H.264.", Senda.colors.textMid)
    }
    ...
    Text("Una vista previa más lenta ahorra batería y calor; no afecta al directo.", style = Senda.numeric, color = Senda.colors.textLow)
    SwitchRow(
...
private fun ThermalSection(s: StudioSettings, thermal: ThermalProfile, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    val t = s.thermal
    Header("TEMPERATURA")
    Callout("Estado actual: ${thermal.level.label}" + (thermal.message?.let { "\n$it" } ?: ""), if (thermal.throttled) Senda.colors.record else Senda.colors.textMid)
...
private fun RecordingSection(s: StudioSettings, onUpdate: ((StudioSettings) -> StudioSettings) -> Unit) {
    ...
    Text("La grabación usa el mismo codificador que el directo: calidad idéntica y sin calor extra.", style = Senda.numeric, color = Senda.colors.textLow)
}
...
private fun AboutSection(onOpen: (String) -> Unit) {
    ...
    Header("ACERCA DE")
    Text("Senda Studio $version", color = Senda.colors.textHigh)
    Text(
    ...
    Row(
        Modifier
            .fillMaxWidth()
            .background(Senda.colors.panel, RoundedCornerShape(Senda.metrics.radius))
            .clickable { onOpen(SPONSOR_URL) }
            .padding(16.dp),
        verticalAlignment = Alignment.CenterVertically,
    ) {
        Icon(Icons.Outlined.Favorite, null, tint = Senda.colors.live, modifier = Modifier.size(24.dp))
        Column(Modifier.padding(start = 12.dp).weight(1f)) {
            Text("Apoyar el proyecto", color = Senda.colors.textHigh)
            Text("Senda Studio es gratis y sin anuncios. Tu apoyo en GitHub Sponsors mantiene el desarrollo.", style = Senda.numeric, color = Senda.colors.textLow)
        }
    }
}
...
private fun LinkRow(title: String, url: String, onOpen: (String) -> Unit) {
    Row(Modifier.fillMaxWidth().clickable { onOpen(url) }.padding(vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
        Text(title, color = Senda.colors.accentText, modifier = Modifier.weight(1f))
        Icon(Icons.AutoMirrored.Outlined.OpenInNew, null, tint = Senda.colors.accentText, modifier = Modifier.size(18.dp))
    }
}

@Composable
private fun Header(text: String) {
    Text(text, style = Senda.panelLabel, color = Senda.colors.accentText, modifier = Modifier.padding(top = 12.dp))
}
