...
import com.senda.studio.ui.destinations.DestinationEditor
import com.senda.studio.ui.destinations.DestinationsPanel
import com.senda.studio.ui.destinations.DestinationsUi
import com.senda.studio.ui.destinations.DestinationsViewModel
import com.senda.studio.ui.destinations.PrimaryAction
import com.senda.studio.ui.settings.SettingsScreen
...
fun StudioScreen(vm: StudioViewModel, destinationsVm: DestinationsViewModel, onRequestPermissions: () -> Unit) {
    ...
    Scaffold(containerColor = Senda.colors.ink, snackbarHost = { SnackbarHost(snackbar) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding)) {
            if (missingPermissions) {
                Row(Modifier.padding(8.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Column(Modifier.weight(1f)) { Callout("Sin permiso de cámara o micrófono no se puede capturar.", Senda.colors.record) }
...
private fun SourceProperties(vm: StudioViewModel, state: StudioState, sourceId: String, screenCaptureActive: Boolean, onRequestScreenCapture: () -> Unit) {
    ...
    val error = (sourceStatus[sourceId] as? com.senda.studio.engine.capture.CaptureStatus.Error)?.message ?: audioErrors[sourceId]
...
private fun CanvasArea(state: StudioState, vm: StudioViewModel, modifier: Modifier) {
    ...
    Row(modifier, horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        Column(Modifier.weight(1f)) {
            Text("PREVIO", style = Senda.panelLabel, color = Senda.colors.accentText, modifier = Modifier.padding(bottom = 4.dp))
            PreviewCanvas(
                canvas = state.canvas,
                ...
                onSelect = vm::selectItem,
                onTransform = vm::setTransform,
                frameColor = Senda.colors.accent,
            )
        }
        Column(Modifier.weight(1f)) {
            Text("PROGRAMA", style = Senda.panelLabel, color = Senda.colors.live, modifier = Modifier.padding(bottom = 4.dp))
            PreviewCanvas(
                canvas = state.canvas,
                scene = state.programScene,
                sources = state.sources,
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(enabled = enabled, onClick = { onConfirm(); onDismiss() }) {
                Text(confirmLabel, color = Senda.colors.live)
            }
        },
