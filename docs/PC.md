...
Sirga Studio recibe la pantalla y el sonido de tu ordenador y los mezcla con la cámara, los textos y el
micrófono del móvil. El móvil hace el directo; el PC solo juega y envía la señal con **Sirga Studio PC**,
una aplicación propia para Windows.

```
PC (juego + Sirga Studio PC) ──WiFi o cable USB──▶ móvil (Sirga Studio: escenas + emisión) ──▶ Twitch, YouTube…
```

## Cómo se usa

1. **En el móvil:** Fuentes → **+** → **PC (Sirga Studio PC)**. En sus propiedades verás un **código de 4 cifras**.
2. **En el PC:** descarga `SirgaStudioPC.exe` desde la [última versión](https://github.com/lopezdevf/sirga-studio/releases/latest)
...
Si el móvil se desconecta (sales de la app, se corta la WiFi…), Sirga Studio PC lo vuelve a intentar cada
...
Sirga Studio PC mide el retraso real: el móvil devuelve la marca de cada fotograma que muestra y el PC
...
- Comprueba que Sirga Studio está abierto y que la escena con la fuente PC está en pantalla.
...
- Si Windows pregunta por el cortafuegos al abrir Sirga Studio PC, permite el acceso en redes privadas.
...
El resultado es `pc\build\SirgaStudioPC.exe`, un único archivo de unos 370 KB.

## Protocolo Sirga Link

Documentado en [`engine/.../pclink/SirgaLink.kt`](../engine/src/main/java/com/sirga/studio/engine/pclink/SirgaLink.kt):
TCP con enteros big-endian, saludo con código de emparejamiento, vídeo H.264 Annex-B, audio PCM de 16
bits y búsqueda por difusión UDP en el puerto 9750. [`pc/tools/sirga_link_sender.py`](../pc/tools/sirga_link_sender.py)
