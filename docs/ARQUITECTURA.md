...
| `:app`    | UI en Jetpack Compose (mesa de control, propiedades, ajustes), tema Sirga, ViewModels. |
...
## Enlace con el PC (Sirga Link)

La fuente «PC» recibe la pantalla y el sonido de **Sirga Studio PC** (`pc/`, C++ con el SDK de Windows) por
...
flowchart LR
    ...
    MFT --> TCP[Sirga Link por TCP]
    TCP --> DEC[MediaCodec<br/>sin búfer de espera]
    DEC --> COMP[Textura del compositor]
    TCP --> MIX[Canal del mezclador]
```

- **Descubrimiento**: el PC envía `SGL1?` por difusión UDP (puerto 9750) y cada fuente PC activa responde
...
7. **Validación en dispositivos** — probado en un Galaxy S25 Ultra (cámaras, grabación, Twitch 1080p60, fuente PC con
   Sirga Studio PC por WiFi); faltan más fabricantes, cámaras UVC y capturadoras.
