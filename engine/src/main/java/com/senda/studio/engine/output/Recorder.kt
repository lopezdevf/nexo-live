...
/**
 * Graba el mismo vídeo y audio codificados que se emiten en un MP4 de Movies/SendaStudio.
...
class Recorder(context: Context) {
    ...
    fun deleteAbandoned() {
        ...
        val args = Bundle().apply {
            ...
            putString(ContentResolver.QUERY_ARG_SQL_SELECTION, "${MediaStore.Video.Media.DISPLAY_NAME} LIKE 'SendaStudio_%'")
        }
        runCatching {
    ...
    fun start(folder: String, expectAudio: Boolean, video: MediaFormat?, audio: MediaFormat?): String = synchronized(lock) {
        ...
        val name = "SendaStudio_" + SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ROOT).format(Date()) + ".mp4"
