...
/**
 * Graba el mismo vídeo y audio codificados que se emiten en un MP4 de Movies/NexoLive.
...
class Recorder(context: Context) {
    ...
    fun start(folder: String, expectAudio: Boolean, video: MediaFormat?, audio: MediaFormat?): String = synchronized(lock) {
        ...
        val name = "NexoLive_" + SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ROOT).format(Date()) + ".mp4"
